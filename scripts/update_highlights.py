...
daily quota is 10,000 — about 100 searches/day. Antonelli's career is ~35
races so the whole backfill fits in a single run (the script still stops
cleanly at the limit and resumes where it left off — idempotent by
design). After the backfill, a weekly run needs only 1–2 searches.
...
USER_AGENT = "AK12-fan-site-data-script/0.1 (unofficial Antonelli fan site)"
