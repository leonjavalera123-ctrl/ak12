"""Fetch openly licensed Kimi Antonelli images from Wikimedia Commons.
...
Usage:  python scripts/fetch_images.py            # default queries
        python scripts/fetch_images.py "Antonelli Monza"   # custom query
"""
...
USER_AGENT = "AK12-fan-site/0.1 (unofficial Antonelli fan site; contact in repo README)"
...
# Some Commons files about a person are not photographs OF the person and
# should never land on a fan site. A handwritten signature is the clear
# case: Commons hosts it as public domain for encyclopedic infoboxes, but a
# clean high-resolution copy is a forgery template and this site has no use
# for it. Rejected by title before the license check even runs.
REJECT_TITLE = re.compile(r"signature|autograph", re.I)


def evaluate(page: dict) -> tuple[dict, str] | tuple[None, str]:
    """Return (manifest_entry, 'ok') for an acceptable image, else (None, reason)."""
    if REJECT_TITLE.search(page["title"]):
        return None, "signature/autograph image — never appropriate here"
