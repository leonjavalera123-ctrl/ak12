"""Pull Kimi Antonelli's complete F1 race history from the Jolpica API and
...
def to_number(text):
    ...
    are awarded for shortened races), so keep floats only when fractional."""
    value = float(text)
...
def fetch_standing_after(season: int, rnd: int) -> dict | None:
    ...
    for entry in lists[0]["DriverStandings"]:
        if entry["Driver"]["driverId"] == DRIVER_ID:
            ...
            # and the "position" key is absent.
...
def build_upcoming_entry(race: dict) -> dict:
    """A scheduled race that has no Antonelli result yet."""
    return {
...
def main() -> int:
    ...
    for season in sorted(seasons):
        ...
        # Teams in the order he drove for them that season (a mid-season
        # switch would mean a season can have two).
